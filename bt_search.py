...
ciliwang = {
    "磁力猫": "https://clm322.buzz/",
    "搜磁力": "https://soucili.info/",
    "种子搜": "https://m.zhongziso98.xyz/",
    "磁力蜘蛛": "https://clzz1024.buzz/",
}


async def clm_beiyong():
    reps = await aiorequests.get("https://xn--j6x4d.com/", timeout=6)
...
async def clm_crawler(tag):
    ...
    try:
        geturl = await aiorequests.get(f"{url}search-{tag}-0-0-1.html", timeout=6)
    except:
...
async def zzs_crawler(tag):
    ...
    try:
        geturl = await aiorequests.get(f"{url}list/{tag}/1", timeout=6)
    except:
        return []
    if geturl.status_code != 200:
        return "bad"
    ...
    for table in table_list:
        ...
        for content in info_dl:
            if content != "\n" and "text-type" not in content.get("class", ""):
                info_text += f"{content.text} "
...
async def clzz_crawler(tag):
    ...
    title_list, magnet_list, info_list = [], [], []
    soup = bs(reqs, "lxml")
    item_list = soup.findAll(name="div", class_="ssbox")
    for item in item_list:
        if item.text == "没有找到记录！":
            return title_list
        itemtitle = item.find(name="div", class_="title")
        itembar = item.find(name="div", class_="sbar")
        title = itemtitle.a.text
        title_list.append(title.replace(tag, f"『{tag}』"))
        magnet = itemtitle.a.get("href").replace("/hash/", "magnet:?xt=urn:btih:").replace(".html", "")
        magnet_list.append(magnet)
...
async def soucili(tag):
    ...
    geturl = await aiorequests.get(
        f"{url}request/?request_type=esearch&keywords={tag}&device_id={device_id}&page=0", headers=soucilihead
    )
    if geturl.status_code != 200:
        stat = "bad"
        return stat
...
def mes_creater(data):
    ...
    mes_list = []
    mes_list.append({"type": "node", "data": {"name": "神秘代码机器人", "uin": "2854196310", "content": f"以下搜索结果来自于{url}"}})
